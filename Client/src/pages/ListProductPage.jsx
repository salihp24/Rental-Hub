...
const emptyImageRow = () => ({
  url: "https://images.unsplash.com/photo-1526170375885-4d8edc77b100?w=800&q=80",
  publicId: "demo-listing-cover",
});
...
function validateSlabsForSubmit(slabs) {
  ...
  for (let i = 0; i < slabs.length; i++) {
    const s = slabs[i];
    const minDays = Number(s.minDays);
    const maxDays = Number(s.maxDays);
  ...
  for (let i = 1; i < sorted.length; i++) {
    if (sorted[i].minDays <= sorted[i - 1].maxDays) {
      return {
        error:
          "Pricing slabs must not overlap: each slab’s minimum day must be greater than the previous slab’s maximum day.",
        slabs: null,
      };
    }
  }
...
export default function ListProductPage() {
  ...
  const handleCategoryChange = useCallback((id, node) => {
  ...
  const onSubmit = async (e) => {
    ...
    for (const img of imagePayload) {
      if (!/^https:\/\//i.test(img.url)) {
        setError("Each image URL must use HTTPS (required by the API).");
        return;
      }
    }
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-black md:text-3xl">
            ...
            {/* This form matches the API: daily pricing, optional discount slabs, location, and
            images (HTTPS URLs with a public ID, e.g. from your image host). */}
      ...
      >
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[11px] font-semibold text-black/45">
              Pick the most specific node (e.g. Samsung under Android). Product browse filters include
              everything under a parent category.
            </p>
            <CategoryTreeSelect
              value={categoryId}
        ...
        <section className="space-y-4">
          <div className="text-sm font-extrabold text-black">Images</div>
            ...
            {/* At least one image. URLs must be <span className="font-extrabold">https://</span>.
            Public ID is any stable string your host uses (e.g. Cloudinary public_id). */}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <div className="text-[11px] font-semibold text-black/45">
                  If rent length falls in a range, that discount % applies to (daily rate × days).
                  Ranges must not overlap; use 9999 as max for “and above”.
                </div>
              </div>
